...
\begin{equation}
	x_\mathrm{len} = \left(2\sin\alpha_h\right)\left(1-\frac{d_h}{f_h}\right)x_L + 2\left[\left(1-\frac{d_h}{f_h}\right)r_{\mathrm{lens},h},+d_h\right]\theta_Y\,,
	\label{eqn:x_len}
\end{equation}
where $d_h$ is the length between the lens and the length-sensing QPD onto the horizontal plane, and $f_h$ is the focal length of the lens projected onto the horizontal plane\footnote{Thanks Ushiba-san for pointing out the necessity of projecting the focal length onto the horizontal plane in addition to the lever arms \cite{comment_to_setup_of_length_sensing_oplev_for_itmx}.}.
Here, we introduce an angle $\theta_h$ between the horizontal plane and the optical lever beam $\vec{r}_\mathrm{lens}$ such that the horizontal arm is
\begin{equation}
	r_{\mathrm{lens},h} = \left\lvert \vec{r}_\mathrm{lens} \right\rvert\cos{\theta_h}\,.
	\label{eqn:r_len_horizontal_projection}
\end{equation}
The same goes to the length between the length-sensing QPD and the lens, as well as for the focal length,
\begin{equation}
	d_h = \left\lvert \vec{r}_\mathrm{lens} \right\rvert\cos{\theta_h}\,,
	\label{eqn:d_horizontal_projection}
\end{equation}
and
\begin{equation}
	f_h = f\cos{\theta_h}\,.
	\label{eqn:focal_length_horizontal_projection}
\end{equation}
Substituting Eqn.~\eqref{eqn:r_len_horizontal_projection}, \eqref{eqn:d_horizontal_projection}, and \eqref{eqn:focal_length_horizontal_projection} into \eqref{eqn:x_len} yields
\begin{equation}
	x_\mathrm{len} = \left(2\sin\alpha_h\right)\left(1-\frac{d}{f}\right)x_L + 2\cos\theta_h\left[\left(1-\frac{d}{f}\right)r_{\mathrm{lens}}+d\right]\theta_Y\,.
	\label{eqn:x_len_projection_substituted}
\end{equation}
So, it follows that we need to put the length-sensing QPD at 
\begin{equation}
	d = \frac{r_\mathrm{lens}f}{r_\mathrm{lens}-f}\,,
\end{equation}
which is identical to Eqn.~\eqref{eqn:d}, the result we have derived in Sec.~\ref{sec:very_basic_derivation}.
Coincidentally, the vertical readout $y_\mathrm{len}$ will be decoupled from pitch $\theta_P$.

Meanwhile, the vertical beam spot displacement at the length-sensing QPD reads
\begin{equation}
		y_\mathrm{len} = \left(2\sin\alpha_v\right)\left(1-\frac{d_v}{f_v}\right)x_L + 2\left[\left(1-\frac{d_v}{f_v}\right)r_{\mathrm{lens},v}+d_v\right]\theta_P\,,
	\label{eqn:y_len}
\end{equation}
where $f_v$ is vertical projection of the focal length $f$.
...
If we ensemble Eqn.~\eqref{eqn:x_tilt}, \eqref{eqn:y_tilt}, \eqref{eqn:x_len}, and \eqref{eqn:y_len} into a matrix form, we get
\begin{equation}
	\begin{pmatrix}
		x_L\\
		\theta_P\\
...
\begin{equation}
	...
	\begin{bmatrix}
		...
		2\sin\alpha_h\left(1-\frac{d_h}{f_h}\right) & 0 & 2\left[\left(1-\frac{d_h}{f_h}\right)r_{\mathrm{lens},h}+d_h\right]\\
		2\sin\alpha_v\left(1-\frac{d_v}{f_v}\right) &  2\left[\left(1-\frac{d_v}{f_v}\right)r_{\mathrm{lens},v}+d_v\right] & 0\\
	\end{bmatrix}^{+},
\end{equation}
...
The vertical readout $y_\mathrm{len}$ of the length-sensing QPD is ideally a null stream.
So, we can choose to omit the readout $y_\mathrm{len}$ such that the sensing matrix becomes full-rank.
Hence, the sensing matrix for a horizontal optical lever is
\begin{equation}
	\begin{pmatrix}
		x_L\\
		\theta_P\\
	...
	\begin{bmatrix}
		2\sin\alpha_h & 0 & 2r_h\\
		0 & 2r_v & 0\\
		\frac{-2f\sin\alpha_h}{r_\mathrm{lens}-f} & 0 & 0
	\end{bmatrix}^{-1}
	\begin{pmatrix}
		x_\mathrm{tilt}\\
		y_\mathrm{tilt}\\
...
\end{equation}
where we've assumed the placement of the length-sensing QPD is  exactly at $d=d_h=\frac{r_\mathrm{lens}f}{r_\mathrm{lens}-f}$ from the convex lens.
...
\begin{equation}
	...
	\begin{bmatrix}
		0 & 0 & 2r_h\\
		2\sin\alpha_v & 2r_v & 0\\
		\frac{-2f\sin\alpha_v}{r_\mathrm{lens}-f} & 0 & 0
	\end{bmatrix}^{-1}
	\begin{pmatrix}
		x_\mathrm{tilt}\\
		y_\mathrm{tilt}\\
...
\end{equation}
where we've set $d=d_v=\frac{r_\mathrm{lens}f}{r_\mathrm{lens}-f}$.
...
There are two things we can infer from Eqn.~\eqref{eqn:x_len_misplaced_qpd}.
